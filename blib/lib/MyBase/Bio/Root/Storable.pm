#
# BioPerl module for MyBase::Bio::Root::Storable
#
...
MyBase::Bio::Root::Storable - object serialisation methods

=head1 SYNOPSIS

  my $storable = MyBase::Bio::Root::Storable->new();

  # Store/retrieve using class retriever
  my $token     = $storable->store();
  my $storable2 = MyBase::Bio::Root::Storable->retrieve( $token );
...
$MyBase::Bio::Root::Storable::BINARY to false.
...
# Let the code begin...
package MyBase::Bio::Root::Storable;
...
=head2 new
  ...
  Function  : Builds a new MyBase::Bio::Root::Storable inhereting object
  Returntype: MyBase::Bio::Root::Storable inhereting object
  Exceptions: 
  Caller    : 
  Example   : $storable = MyBase::Bio::Root::Storable->new()
...
=head2 statefile
  ...
  Returntype: string
  Exceptions: 
  Caller    : MyBase::Bio::Root::Storable->store
...
sub statefile{
  ...
  if( ! $self->{$key} ){ # Create a new statefile
    ...
    my( $fh, $file ) = MyBase::Bio::Root::IO->new->tempfile( @args );
...
#  $self->{$key} ||= $MyBase::Bio::Root::IO::TEMPDIR;
...
sub store{
  ...
  my $io = MyBase::Bio::Root::IO->new( ">$statefile" );
  $io->_print( $store_obj );
...
sub serialise{
  ...
  foreach my $key( keys( %$self ) ){
    ...
    # MyBase::Bio::Root::Storable obj: save placeholder
    elsif( ref($value) =~ /^MyBase::Bio::/ and $value->isa('MyBase::Bio::Root::Storable') ){
      # MyBase::Bio::Root::Storable
      $store_obj->{$key} = $value->new_retrievable( %retargs );
      next;
    }

    # Arrayref value. Look for MyBase::Bio::Root::Storable objs
    elsif( ref( $value ) eq 'ARRAY' ){
      my @ary;
      foreach my $val( @$value ){
	if( ref($val) =~ /^MyBase::Bio::/ and $val->isa('MyBase::Bio::Root::Storable') ){
    ...
    # Hashref value. Look for MyBase::Bio::Root::Storable objs
...
=head2 retrieve
  ...
  Exceptions: 
  Caller    : 
  Example   : my $obj = MyBase::Bio::Root::Storable->retrieve( $token );
...
=head2 _freeze
  ...
  Function  : Converts whatever is in the the arg into a string.
              ...
              depending on the value of $MyBase::Bio::Root::BINARY
  Returntype: 
  Exceptions: 
  Caller    : 
  Example   : 
...
=head2 _thaw
  ...
  Function  : Converts the string into a perl 'whatever'.
              ...
              value of $MyBase::Bio::Root::BINARY.
