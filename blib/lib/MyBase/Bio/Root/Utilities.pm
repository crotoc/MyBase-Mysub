package	MyBase::Bio::Root::Utilities;
use strict;


=head1 NAME

MyBase::Bio::Root::Utilities - General-purpose utility module
...
=head2 Object Creation
    ...
    use MyBase::Bio::Root::Utilities qw(:obj);
    $Util->some_method();

    # Create an object manually:
    use MyBase::Bio::Root::Utilities;
    my $util = MyBase::Bio::Root::Utilities->new();
    $util->some_method();
...
namespace representing a MyBase::Bio::Root::Utilities object. This saves you
from creating your own MyBase::Bio::Root::Utilities object via
C<MyBase::Bio::Root::Utilities-E<gt>new()> or from prefixing all method calls with
C<MyBase::Bio::Root::Utilities>, though feel free to do these things if desired.
Since there should normally not be a need for a script to have more
than one MyBase::Bio::Root::Utilities object, this module thus comes with it's
own singleton.
...
Inherits from L<MyBase::Bio::Root::Root>, and uses L<MyBase::Bio::Root::IO> 
and L<MyBase::Bio::Root::Exception>.
...
use base qw(MyBase::Bio::Root::Root Exporter);
@EXPORT_OK   = qw($Util);
...
#------------
    ...
    if(system(@cmd) != 0) {
        $self->throw( -class => 'MyBase::Bio::Root::SystemException',
...
#---------------------
	...
	$client->{'_input_type'} = "Glob";
        ...
        $self->throw(-class=>'MyBase::Bio::Root::IOException',
	...
	$file = "$gzip -cd $file |"
      ...
      require FileHandle;
      $FH = FileHandle->new();
      open ($FH, $file) || $self->throw(-class=>'MyBase::Bio::Root::FileOpenException',
...
=head2 get_newline
 ...
 Purpose   : Determine the character(s) used for newlines in a given file or
           : input stream. Delegates to MyBase::Bio::Root::Utilities::get_newline()
 Example   : $data = $object->get_newline(-CLIENT => $anObj,
           :                                   -FILE =>'usr/people/me/data.txt')
 Argument  : Same arguemnts as for create_filehandle().
 Returns   : Reference to a FileHandle object.
 Throws    : Propogates any exceptions thrown by MyBase::Bio::Root::Utilities::get_newline().
...
#---------------
  ...
  if($@ =~ /Timed out!/) {
    $self->throw("Timed out while waiting for input.",
		 "Timeout period = $TIMEOUT_SECS seconds.\nFor longer time before timing out, edit \$TIMEOUT_SECS in MyBase::Bio::Root::Utilities.pm.");
...
=head2 file_flavor
 ...
 Throws    : Exception if argument is not a file
           : Propogates any exceptions thrown by MyBase::Bio::Root::Utilities::get_newline().
...
=head2 mail_authority
 ...
 Purpose  : Syntactic sugar to send email to $MyBase::Bio::Root::Global::AUTHORITY
